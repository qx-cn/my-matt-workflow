...
宿主 policy 允许模型隐式选择的 Skill。**描述**帮助候选匹配，但不是调用开关；真正的开关是目标宿主的 invocation policy。模型调用用候选上下文与误路由风险交换自主发现能力。

*避免使用*：能力、工具、功能

### 用户调用

宿主 policy 要求显式选择的 Skill。它仍保留准确的**描述**供 UI、搜索和人工判断使用，但描述不会绕过 policy 触发模型调用。其他 Skill 若需复用其方法，依赖 composition 与 composed-body；若需把下一阶段交给用户，输出可移植调用宏。

*避免使用*：流程、工作流、命令

### 描述

Skill 的宿主可见能力摘要。宿主允许隐式调用时，它参与候选匹配；宿主禁止隐式调用时，它服务 UI、搜索和人工选择。它影响发现质量与候选**上下文负荷**，但其存在本身不决定调用模式。
...
候选 Skill 的**描述**进入 Agent 上下文时造成的 token 与注意力成本。宿主如何选择和加载候选决定实际成本；不能通过删除用户调用 Skill 的准确摘要来假装成本为零。
...
一项**用户调用** Skill，它的工作是指向其他显式入口——逐一说明其名称及何时抵达——使人类只需记住一个入口。它只输出下一跳的可移植调用宏并停止，不执行目标 Skill。这是用户调用 Skill 增多时**认知负荷**的解法。
