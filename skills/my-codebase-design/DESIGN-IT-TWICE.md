...
仅当宿主提供 Agent 工具、用户未禁止、方案探索可以独立切分且并行确实能改善质量或时间时，才并行启动子代理；数量由独立设计空间、风险与当前并发容量决定，不设固定下限。工具不可用或并行收益很低时，由当前 Agent 串行生成独立方案，不得因此停摆。子代理结果只是候选证据，当前 Agent 负责检查约束、处理冲突和最终比较。

为每个子代理提供单独的技术简报（文件路径、耦合细节、来自 [DEEPENING.md](DEEPENING.md) 的依赖类别、Seam 后面有什么）。该简报独立于步骤 1 面向用户的问题空间说明。为每个代理指定不同的设计约束：

- 约束 A：“最小化 Interface——最多以 1–3 个入口点为目标。最大化每个入口点的 Leverage。”
- 约束 B：“最大化灵活性——支持许多用例和扩展。”
- 约束 C：“为最常见的调用者优化——让默认情形变得微不足道。”
- 约束 D（如适用）：“围绕 Ports & Adapters 设计，以处理跨 Seam 依赖。”
