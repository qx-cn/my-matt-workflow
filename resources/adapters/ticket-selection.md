...
实施候选必须同时是 `ticket_kind: implementation`、`status: ready-for-agent | revalidated`、未认领、每个 `blocked_by` Ticket 的 `status: complete`，并至少有一个未勾选的验收复选框。它还必须声明 `execution_agent: auto | codex | cursor | claude`、非空 `rule_sources`、`rule_scope`、`rule_constraints`、`spec_id`、正整数 `spec_revision`、可定位的 `spec_ref`，且 `rule_conflicts: []`；可选 `review_probes` 只能声明 `recovery` 与 `unknown-response`，由 Spec 的持久化恢复或未知外部响应风险决定。实施前运行 `workflow.py validate-ticket <ticket-path>` 验证。`wayfinder-decision` 或任何 `wayfinder:*` 标签永远不得进入实施，即使其他字段看似就绪。引用只可按唯一的 id、路径或标题解析；孤儿、歧义和依赖环必须先修复。
