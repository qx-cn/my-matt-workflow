...
| --- | --- | --- | --- |
| `SKILL.md` | `152e2c97239affb12a60c5f4a7e74ab546a49ae169688c81f4e2ccc42dafa579` | `17381af6c821d090b1bf6b5d9ea64697c1ce2143c4e6016134cc826449d115d1` | Chinese translation with changed repository-document and write-policy contract. |
...
No upstream content is absent; the three changed sections are adaptation deltas, not restoration gaps.
...
- `agents/openai.yaml#policy.allow_implicit_invocation: false` is local-only and models manual-only invocation.
- The project-configured document locations, personal working copies, and manual-only invocation are potentially valid local adaptations when they preserve the upstream method.
...
2. a resolved term without implementation detail is recorded in the local glossary; and
3. an ADR is drafted only when all three upstream-derived gates hold.

`DomainModelingParityTests` retrieves each cited local constraint and requires exactly one matching outcome per scenario.
