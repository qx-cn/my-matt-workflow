...
def _destination(repo: Path, path: Path) -> Path | None:
    ...
    if len(parts) == 5 and parts[:2] == (".agent", "work"):
        ...
        return (
            ...
            / topic
            / artifact_type
            / f"{artifact_type}-{topic}-{_sort_key(stem)}{extension}"
        )

    if len(parts) == 4 and parts[:2] == (".agent", "handoffs"):
        topic = parts[2]
        artifact_type = "handoffs"
        return (
            repo
            / ".agent"
            / "work"
...
def _tools_candidate(repo: Path, link: str) -> Path | None:
    without_fragment = link.partition("#")[0]
    parts = Path(without_fragment).parts
...
def _rewrite_markdown_links(text: str, replacements: dict[str, str]) -> str:
    def replace(match: re.Match[str]) -> str:
        ...
        if replacement is None:
            return match[0]
        return match[0].replace(target, replacement, 1)
...
def analyze_work_artifacts(repo: Path) -> dict[str, object]:
    ...
    The report uses repository-relative POSIX paths so it can be shown directly
    in a CLI dry-run and remain portable across operating systems.
    """
    repo = repo.resolve()
    ...
    for path in sorted(item for item in agent_dir.rglob("*") if item.is_file()):
        if path == agent_dir / "matt-workflow.md":
            continue
    ...
    moves = [
        {"from": _relative(repo, source), "to": _relative(repo, destination)}
        for source, destination in sorted(moves_by_source.items(), key=lambda item: _relative(repo, item[0]))
    ]
    ...
    conflicts = [
        {
            "destination": _relative(repo, destination),
            "sources": sorted(_relative(repo, source) for source in sources),
            "existing": destination.is_file() and destination not in moves_by_source,
        }
        for destination, sources in sorted(
            sources_by_destination.items(), key=lambda item: _relative(repo, item[0])
        )
            ...
            if parsed is not None:
                ...
                if old_target.is_file():
                    ...
                    link_rewrites.append(
                        {
                            "source": _relative(repo, source),
                            "new_source": _relative(repo, new_source),
                            "link": link,
                            "old_target": _relative(repo, old_target),
                            "new_target": _relative(repo, new_target),
                            "replacement": _replacement_link(new_source, new_target) + fragment,
                        }
                    )
                    continue
                ...
                candidate_repairs.append(
                    {
                        ...
                        "new_source": _relative(repo, new_source),
                        "link": link,
                        "candidate_target": _relative(repo, candidate),
                        "replacement": _replacement_link(new_source, candidate),
                    }
                )

    return {
        "compliant": not moves and not conflicts and not unclassified,
        "moves": moves,
        "deletions": [move["from"] for move in moves],
        "link_rewrites": link_rewrites,
        "candidate_link_repairs": candidate_repairs,
        "conflicts": conflicts,
...
def apply_work_artifact_migration(
    ...
    *,
    ...
    The caller must use ``analyze_work_artifacts`` to show the report before
    invoking this function. Existing destinations stop the operation before
    any file is moved; unclassified artifacts remain in place for manual
    review.
    """
    repo = repo.resolve()
    ...
    if conflicts:
        raise WorkArtifactError(f"迁移目标冲突：{conflicts}")

    moves = report["moves"]
    move_paths = [(repo / move["from"], repo / move["to"]) for move in moves]
    ...
    confirmed_candidate_link_repairs = confirmed_candidate_link_repairs or set()
    candidates_by_key = {
        (candidate["source"], candidate["link"]): candidate
        for candidate in report["candidate_link_repairs"]
    }
    unknown_candidates = confirmed_candidate_link_repairs - candidates_by_key.keys()
    if unknown_candidates:
        raise WorkArtifactError(f"不存在的候选链接修复：{sorted(unknown_candidates)}")

    rewrites = list(report["link_rewrites"])
    rewrites.extend(
        candidates_by_key[key]
        for key in sorted(confirmed_candidate_link_repairs)
    )
    replacements_by_source: dict[str, dict[str, str]] = {}
    for rewrite in rewrites:
        replacements_by_source.setdefault(rewrite["new_source"], {})[
            rewrite["link"]
        ] = rewrite["replacement"]
    ...
    for source, destination in move_paths:
        ...
        replacements = replacements_by_source.get(_relative(repo, destination))
