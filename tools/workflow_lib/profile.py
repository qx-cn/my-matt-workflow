...
def render_profile(config: dict[str, Any], notes: str = "") -> str:
    ...
    lines.extend(
        [
            ...
            "# 长任务协作：预计超过 30 秒、需要多个工具调用或多个执行阶段时，开始前先说明目标与下一步。",
            "# 执行中完成关键阶段、确认重要信息、调整计划或遇到阻塞时立即同步；连续工作最长 60 秒不静默。",
            "# 进度同步必须给出具体结果或证据，不只说“正在处理”；等待外部结果前说明等待项，结束后立即汇报。",
            "# 不展示冗长内部思考；只提供便于监督和纠偏的结论、证据与决策节点；最终答复简要总结完成内容、验证结果与遗留问题。",
            "# 下列值是本项目当前实际生效的策略。",
        ]
    )
    for key in PROFILE_FIELD_ORDER:
